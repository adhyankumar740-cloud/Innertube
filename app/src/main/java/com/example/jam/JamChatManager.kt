...
class JamChatManager {
    ...
    /**
     * [id] is generated by the caller (JamViewModel), not here - the caller adds a matching
     * local ChatMessage to its own state *before* calling this, so sending isn't dependent on
     * the "self" broadcast echo actually making it back over the wire (see JamViewModel.sendMessage
     * for why: it was found not to be reliable - first message after joining a room would show up
     * only because it happened to also land in the one-time history SELECT, every message after
     * that had nothing to fall back on and just never appeared, even to its own sender).
     */
    fun sendMessage(
        id: String,
        senderId: String,
        ...
        scope.launch {
            try {
                // Still broadcast (other participants rely on this to see it live) and persist,
                // but the sender no longer depends on either of these to see their own bubble.
