...
class MusicService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        // post itself.
        // NEVER-GO-AWAY FIX: a plain DefaultMediaNotificationProvider only keeps the
        // notification non-dismissible while actively playing - it becomes swipeable (and on
        // some OEM skins gets auto-cleared from the notification shade/quick settings) the
        // moment playback pauses. AlwaysOngoingMediaNotificationProvider forces it to stay
        // pinned regardless of play state.
        setMediaNotificationProvider(
            AlwaysOngoingMediaNotificationProvider(
                this,
                { NOTIFICATION_ID },
                CHANNEL_ID,
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        // NEVER-GO-AWAY FIX: this used to also stopSelf() whenever playWhenReady was false
        // (i.e. simply paused) - so swiping the app away from Recents while paused killed the
        // session and took the notification down with it. Now the service (and its
        // notification) stays alive across a task-removal as long as a track is actually
        // loaded, whether playing or paused, and only tears down when the queue is genuinely
        // empty.
